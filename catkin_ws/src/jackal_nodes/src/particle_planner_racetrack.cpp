...
track_info closestTrackPoint(float xposition, float yposision)
{
		track_info a;
		a.index = -1;
		a.dis = 1000000000.0;
		...
		{
			float dis = sqrt(pow(track_x[i]-xposition,2)+pow(track_y[i]-yposision,2));
			if(dis < a.dis)
			{
				a.index = i;
				a.dis = dis;
...
track_info closestTrackPoint_relative(float xposition, float yposision, int guess_index)
{
		track_info a;
		a.index = -1;
		a.dis = 1000000000.0;
		for(int i = guess_index-30; i< guess_index+30; i++)
		{
			int ind = i%track_x.size();
			float dis = sqrt(pow(track_x[ind]-xposition,2)+pow(track_y[ind]-yposision,2));
			if(dis < a.dis)
			{
				a.index = i;
				a.dis = dis;
				a.diff = i-guess_index;
			}
		}
		return a;
}

void processFileLine(string& str)
{
	//float num_float = stof(str);
	//char a[] = "123.4567";
	//float num_double = atof(str.c_str());
	//ROS_INFO("cc: %f",num_double);
	for(int i = 0;i<str.length();i++)
	{
		if(str[i]==' ')
		{
			track_x.push_back(atof(str.substr(0,i).c_str()));
			track_y.push_back(atof(str.substr(i+1,str.length()-i-1).c_str()));
		}
	}
}
...
float validate_particle(int rand_num, float curr_yaw, float curr_x, float curr_y, int curr_ind)
{	
	...
	track_info new_i = closestTrackPoint_relative(new_x, new_y,curr_ind);
	new_ind = new_i.index;
	//check validity
	// if valid, update particles[] input (timestep) and particles[] pos (timestep + 1)
	// else, return false
	if(new_i.dis >= track_buffer)
	{
		safe = false;
		weight = track_buffer-new_i.dis+min_w; //nonsafe weights should always be below safe weights
	}
	...
	if(safe)
	{
		//TODO: figure out how to show distance and figure out how to do it quickly
		weight = (new_i.diff) - (abs(ang_z)*en_multiplier); //(new_x-curr_x)-(abs(ang_z)*en_multiplier);
		col = 0;
	}
	else
	{
...
int generate_plan() // generates plan, updates particle 2D vectors on positions (x, y, theta), and inputs (lin.x, ang.z)
{
	...
	vector<p_pos> pos;
	vector<control_input> in;
	struct track_info a = closestTrackPoint(x_pos, y_pos);
	int starting_index = a.index;
	struct particle_info pi = {pos, in, 0.0, 0, num_particles, starting_index};
	...
	for(int j = 0; j<num_timesteps_planning; j++) // for each time step, update enhanced_pos
	{
		...
		for(int i =0; i < particles_path_size; i++) //update current input, and next position of 
		{
			...
			for(int k = 0; k<num_combinations;k++)// either choose distribute particles by a weighting, or evenly choose best X particles 
			{
				...
				float weight = validate_particle(k, p_pop.pos[j].yaw, p_pop.pos[j].x, p_pop.pos[j].y, p_pop.track_index);
				if(weight > 0.0)
				{
					total_weight+=weight;
				}
				...
				struct potential_particle_info ppi = {p1, c1, weight, col, new_ind};
			...
			for(int k =0; k < particles_path_temp_size; k++)
			{
				...
				if(num_part_assign > 0)
				{
					...
					struct particle_info pi = {pos, in, p_pop.energy+en, p_pop.col_num+pot_p_pop.collision, num_part_assign, pot_p_pop.track_index};
	...
	int best_col = 1000000000;
	//TODO: different ways to weight progress vs energy
	//TODO: change to path based progress: get closest current point
	//get closest point for each particle. Look at progress (closest point might reset back to 0)
	//Choose best progress
	for(int i =0; i < particles_path.size(); i++)
	{
		int index_diff = particles_path[i].track_index-starting_index;
		if(starting_index >= -50%track_x.size() && particles_path[i].track_index<=50)
		{
			index_diff = particles_path[i].track_index+track_x.size()-starting_index;
		}
		if(particles_path[i].col_num < best_col)
		{
			best_w = index_diff-particles_path[i].energy;
			best_index = i;
			best_col = particles_path[i].col_num;
		}
		else if(particles_path[i].col_num == best_col && (index_diff-particles_path[i].energy) > best_w)
		{
			best_w = index_diff-particles_path[i].energy;
			best_index = i;
			best_col = particles_path[i].col_num;
...
int main(int argc, char **argv)
{
	...
	ros::init(argc, argv, "particle_planner_racetrack");
	ros::NodeHandle nh;
  ...
  obsyvel = nh.advertise<std_msgs::Float32MultiArray>("/obstacle/y_vel", 1, true);
	...
	string line;
	ifstream myfile ("track_points_3.txt");
	if (myfile.is_open())
	{
